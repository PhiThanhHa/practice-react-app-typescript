...
export const GlobalReducer = (state: IAppRequestState, action: AnyAction, actionType: string): IAppRequestState | IReportState => {
	switch (action.type) {
		...
		case `${actionType}_SUCCESS`: {
			return {
				...
				total_abuse_report: action.total_abuse_report && action.total_abuse_report,
				total_moderator_report: action.total_moderator_report && action.total_moderator_report,
				total_report: action.total_report && action.total_report,
			};
		}
